...
export class IndexComponent implements OnInit,  AfterViewInit {
  ...
  if(browser!= "ie" ){
    ...
    this.mySubscription = this.IntrestForm
    .get('accountConfirmation')
    .valueChanges   
    .subscribe(newValue => {
if (newValue === 'no') {
// place is not required anymore
this.IntrestForm.get('accountNo').setValidators([]);
} else {
// accountNo is required
this.IntrestForm.get('accountNo').setValidators([Validators.required,Validators.minLength(12)]);
console.log(this.IntrestForm.get('accountNo').value)
}
// force valitators to be triggered, to update form validity.
this.IntrestForm.get('accountNo').updateValueAndValidity();
});
  ...
  numberOnly(event): boolean {
    ...
    if ((event.target.value.substr(4, 1) == 0 || event.target.value.substr(4, 1) == 5) && ((event.target.value.substr(5, 1) == 0 || event.target.value.substr(5, 1) == 5)) ) {
      ...
      this.IntrestForm.controls['accountNo'].setErrors({ 'incorrect': true });
      this.showerror=true;
        }
    return true;
  }
  ...
  inter_submit() {
    ...
    var feedback =
      ...
      this.Inter_contactnumber;
        ...
        var accountNo = btoa(this.IntrestForm.controls['accountNo'].value)
       ...
       //   let accountNo = btoa(this.accountNo2.toString())
              /* DB insertion */
              var json2 = {
