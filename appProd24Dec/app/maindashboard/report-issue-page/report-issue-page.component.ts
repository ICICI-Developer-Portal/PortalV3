...
export class ReportIssuePageComponent implements OnInit {
    ...
    keyPressAlphaNumericSpace(event) {
    ...
    readFile(fileEvent: any) {
        ...
        const allowed_types = ['pdf', 'zip', 'xlsx', 'jpeg', 'png', 'jpg','xls'];
        ...
        if (fileEvent.target.files && fileEvent.target.files[0]) {
            if (file.size > 1048576) { // if file is grtr thn 1mb
                ...
                if ((allowed_types).includes(ext)) {
                    ...
                    $(".fileError").text(ext + ' File type not allowed. Only pdf,zip,xls,xlsx,jpeg,png,jpg file type are  allowed')
                    $(".fileError").show();
                    return false;
          ...
          /*   r.onload = function(e) { 
                self.Attach = r.result;
                console.log(self.Attach);
              }
 ...
 */
    ...
    //    console.log(json)
       // let jsondataPacket = { issuePacket: json };
        ...
        formData.append("product", inputFields["product"]);
        formData.append("environment", inputFields["environment"]);
        formData.append("api", inputFields["api"]);
        formData.append("issueType", inputFields["issueType"]);
        formData.append("issueId", inputFields["issueId"]);
        formData.append("team", inputFields["team"]);
        formData.append("severity", inputFields["severity"]);

        let a: any = (<HTMLInputElement>document.getElementById("Attachments")).files;
        for (let k = 0; k < a.length; k++) {

            formData.append("Attach", a[k]); //34
    
          }
       // formData.append("Attach", inputFields["Attach"]);




        formData.append("FileName", inputFields["FileName"]);
        formData.append("FileType", inputFields["FileType"]);
        formData.append("ContentType", inputFields["ContentType"]);
        formData.append("contactPerson", inputFields["contactPerson"]);
        formData.append("contactNo", inputFields["contactNo"]);
        formData.append("userID", inputFields["userID"]);
        formData.append("issueDescription", inputFields["issueDescription"]);
        formData.append("endpointURL", inputFields["endpointURL"]);
        formData.append("issueFirstObserbedDate", inputFields["issueFirstObserbedDate"]);
        formData.append("requestPacket", inputFields["requestPacket"]);
        formData.append("responsePacket", inputFields["responsePacket"]);
        formData.append("errorMessage", inputFields["errorMessage"]);
        formData.append("notes", inputFields["notes"]);
        formData.append("contactEmail", inputFields["contactEmail"]);
        formData.append("username", localStorage.getItem('username'));
        console.log(formData);
       /*  this.adm.raiseSRRequest(inputFields).subscribe(
            (data: any) => { 
               
                console.log(data);
            //    let resp = this.decode(data);
                console.log(data._body);
        ...
        this.HttpClient.post<any>(
            ...
            "https://developer.icicibank.com/rest/reportIssue",
            ...
            data => {
                console.log(data);
                //    let resp = this.decode(data);report
