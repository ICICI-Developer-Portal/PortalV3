...
this.adm.Login(json).subscribe((data: any) => {
  ...
  sessionSet(key, value, expirationInMin = 20) {
    ...
    localStorage.setItem("expirationDate",JSON.stringify({value:expirationDate.getTime()}));
  }
/* 
var now = new Date();
    let expirationDate = this.AddMinutesToDate(now,2);
    let newValue = {
      value: value,
      expirationDate: expirationDate.togetimtoString()
    };
    window.sessionStorage.setItem(key, JSON.stringify(newValue));
  }
