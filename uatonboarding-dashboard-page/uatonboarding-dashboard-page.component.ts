...
ifIPpatternNotmatches(){
  ...
  if ($(".customcsscontainer input:checkbox:checked").length > 0) { $("#thrdSectionChld").removeClass("overlay_parent"); $("#submitButton,#file1").removeClass("blockElements");   $('#nav-tab a[href="#tab3"]').tab('show'); this.setProgress(2);
  ...
  selectAll(e, isChecked: boolean){
    alert(e.target.getAttribute('parentName'))
    alert(e.target.checked)
    alert( $(".customcsscontainer input[parentname='"+e.target.getAttribute('parentName')+"']"))
  ...
  multipleSelectAPI(e, isChecked: boolean){
   this.forResetiingAdditionalFields();
    ...
    if($(".customcsscontainer input[parentname='"+e.target.getAttribute('parentName')+"']").length==$(".customcsscontainer input[parentname='"+e.target.getAttribute('parentName')+"']:checkbox:checked").length){
      $(".selectAll input[parentname='"+e.target.getAttribute('parentName')+"']").attr("checked",true);
    }
    else{
      alert("hii")
      $(".selectAll input[parentname='"+e.target.getAttribute('parentName')+"']").removeAttr("checked");
